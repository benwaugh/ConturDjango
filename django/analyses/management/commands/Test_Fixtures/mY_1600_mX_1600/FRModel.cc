...
FRModel::FRModel()
: ZERO_(),
gYXm_(1.0),gYq_(0.25),aEWM1_(127.9),Gf_(1.16637e-05),aS_(0.1184),
ymb_(4.7),ymt_(172),ymtau_(1.777),MZ_(),MTA_(),
MT_(),MB_(),MH_(),MXm_(),MY1_(),
WZ_(),WW_(),WT_(),WH_(),WXm_(),
WY1_(),aEW_(),G_(),MW_(),ee_(),
sw2_(),cw_(),sw_(),g1_(),gw_(),
vev_(),lam_(),yb_(),yt_(),ytau_(),
muH_(),I1a33_(0.0,0.0),I2a33_(0.0,0.0),I3a33_(0.0,0.0),I4a33_(0.0,0.0)
...
void FRModel::doinit() {
     ZERO_ = 0.0;
   resetMass(3,ZERO_ * GeV);
   MZ_ = getParticleData(23)->mass()  / GeV;
   MTA_ = getParticleData(15)->mass()  / GeV;
   MT_ = getParticleData(6)->mass()  / GeV;
   MB_ = getParticleData(5)->mass()  / GeV;
   MH_ = getParticleData(25)->mass()  / GeV;
   MXm_ = getParticleData(52)->mass()  / GeV;
   MY1_ = getParticleData(9000006)->mass()  / GeV;
   WZ_ = getParticleData(23)->width() / GeV;
   WW_ = getParticleData(24)->width() / GeV;
   WT_ = getParticleData(6)->width() / GeV;
   WH_ = getParticleData(25)->width() / GeV;
   WXm_ = getParticleData(52)->width() / GeV;
   WY1_ = getParticleData(9000006)->width() / GeV;
   aEW_ = (1.0/aEWM1());
  ...
  writeParamCard();
  setCouplings("DMS",make_pair(3,2));
...
<< sw2_<< cw_<< sw_<< g1_<< gw_
<< vev_<< lam_<< yb_<< yt_<< ytau_
<< muH_<< I1a33_<< I2a33_<< I3a33_<< I4a33_
 ;
}
...
static ClassDocumentation<FRModel> documentation
 ...
 static Parameter<FRModel, double> interfacegYXm
  ("gYXm",
   "The interface for parameter gYXm",
   &FRModel::gYXm_, 1.0, 0, 0,
   false, false, Interface::nolimits);
static Parameter<FRModel, double> interfacegYq
  ("gYq",
   "The interface for parameter gYq",
   &FRModel::gYq_, 0.25, 0, 0,
   false, false, Interface::nolimits);
...
// no interface for MTA. Use particle definition instead.

// no interface for MT. Use particle definition instead.

// no interface for MB. Use particle definition instead.

// no interface for MH. Use particle definition instead.

// no interface for MXm. Use particle definition instead.

// no interface for MY1. Use particle definition instead.
...
// no interface for WH. Use particle definition instead.

// no interface for WXm. Use particle definition instead.

// no interface for WY1. Use particle definition instead.
